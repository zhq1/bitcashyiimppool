...
int VerifyCycle(std::string hashStr, uint8_t edgeBits, uint8_t proofSize, const std::vector<uint32_t>& cycle)
{
	if (cycle.size() != proofSize) 
		return POW_SHORT_CYCLE;
	if (!(edgeBits >= MIN_EDGE_BITS && edgeBits <= MAX_EDGE_BITS)) return POW_NON_MATCHING;
	siphash_keys keys;
	...
	const char *nonce1, const char *nonce2, const char *ntime, const char *nonce, const char *cycles)
{
	...
	if (!strcmp(g_stratum_algo, "lbry")) {
		...
		sprintf(submitvalues->header, "%s%s%s%s%s%s%s", templ->version, templ->prevhash_be, submitvalues->merkleroot_be,
			ntime, templ->nbits, nonce, "1b"/*Edgebits value 27=0x1B for mainnet 16 for testnet*/);
		ser_string_be(submitvalues->header, submitvalues->header_be, 20);//little endian/big endian swap 32 bit
		sprintf(submitvalues->header_be, "%s%s", submitvalues->header_be, "1b");/*Edgebits value 27=0x1B for mainnet 16 for testnet*/		
	}
	...
	char output2[256];
	char allcycles[2000];
	char allcycles_be[2000];
	char allcycles_be2[2000];
	unsigned char allcycles_bin[2000];
	memset(allcycles, 0, sizeof allcycles);
	memset(allcycles_be, 0, sizeof allcycles_be);

	if (strlen((char*)&cycles) < 2000)strcpy((char*)&cycs, cycles);
	pt = strtok((char*)&cycs, ",");
	int i = 0;
	...
	while (pt != NULL) {		
		...
		string_be(output, output2);
		binlify((unsigned char*)&ui, output2);

		cycle.push_back(ui);		
		if (i<1000)
		memcpy((void*)&allcycles[i], output, 8);
		i += 8;
		pt = strtok(NULL, ",");
	}
	...
	ser_string_be((char*)allcycles, (char*)allcycles_be, 42);//little endian/big endian swap 32 bit
	sprintf(submitvalues->header_be, "%s2a%s", submitvalues->header_be, (char*)allcycles_be);

	//printf("Complete header: %s\n", submitvalues->header_be);
	...
	int cycles_len = strlen(allcycles_be2) / 2;
	g_current_algo->hash_function((char *)allcycles_bin, (char *)submitvalues->hash_bin, cycles_len);
	//sha256_double_hash((char *)allcycles_bin, (char *)submitvalues->hash_bin, cycles_len);

//	unsigned char hash_bin_be[2000];

	//ser_string_be((char*)submitvalues->hash_bin, (char*)&hash_bin_be, 32);//little endian/big endian swap 32 bit


	hexlify(submitvalues->hash_hex, submitvalues->hash_bin, 32);
	
	ser_string_be((char*)submitvalues->hash_hex, (char*)submitvalues->hash_be, 8);//little endian/big endian swap 32 bit
	ser_string_be2((char*)submitvalues->hash_be, (char*)submitvalues->hash_hex, 8);
	//string_be(submitvalues->hash_hex, submitvalues->hash_be);

	binlify(submitvalues->hash_bin, submitvalues->hash_hex);	
...
bool client_submit(YAAMP_CLIENT *client, json_value *json_params)
{
	...
	if (json_params->u.array.length == 6) {
		strncpy(cycles, json_params->u.array.values[5]->u.string.ptr, 2000);		
	}
	...
	if(is_decred)
		...
		build_submit_values(&submitvalues, templ, client->extranonce1, extranonce2, ntime, nonce, cycles);
	...
	/*printf("%016llx actual\n", hash_int);
	printf("%016llx target\n", user_target);
	printf("%s nbits\n", templ->nbits);
	printf("%016llx coin\n", coin_target);*/

	if (templ->nbits && !coin_target) coin_target = 0x7FFF000000000000ULL;
